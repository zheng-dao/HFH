...
app.post('/adminUpdateUserAttributes', async (req, res, next) => {
  ...
  if (!req.body.attributes) {
    const err = new Error('attributes is required');
    err.statusCode = 400;
    return next(err);
  }

  try {
    console.log('Attempting to update attribtues.');
    const response = await adminUpdateUserAttributes(req.body.username, req.body.attributes);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
