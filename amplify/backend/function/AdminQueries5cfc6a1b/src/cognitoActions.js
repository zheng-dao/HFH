...
async function adminUpdateUserAttributes(username, attributes) {
  const params = {
    UserPoolId: userPoolId,
    Username: username,
  ...
  console.log(`Attempting to update attributes for ${username}`);
  try {
    const result = await cognitoIdentityServiceProvider.adminUpdateUserAttributes(params).promise();
    console.log(`Updated atttributes for ${username}.`);
    return {
      message: `Updated attributes for ${username}.`,
    };
  } catch (err) {
    console.log(err);
    throw err;
