...
export default function HotelPropertyUpdateForm(props) {
  const {
    id,
    hotelProperty,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...hotelPropertyRecord };
    ...
    setContact_position(cleanValues.contact_position);
    setTelephone(cleanValues.telephone);
    setEmail(cleanValues.email);
    setIs_blacklist(cleanValues.is_blacklist);
    setStatus(cleanValues.status);
    setExtension(cleanValues.extension);
    setErrors({});
  };
  const [hotelPropertyRecord, setHotelPropertyRecord] = React.useState(hotelProperty);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(HotelProperty, id) : hotelProperty;
      setHotelPropertyRecord(record);
    };
    queryData();
  }, [id, hotelProperty]);
  React.useEffect(resetStateValues, [hotelPropertyRecord]);
  const validations = {
    name: [],
    address: [],
    address_2: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            HotelProperty.copyOf(hotelPropertyRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, 'HotelPropertyUpdateForm')}
    >
      <TextField
        label="Name"
        isRequired={false}
