...
import { getStay } from '@src/customQueries/getStay';
...
export default function AdminApplicationPdf(props) {
  ...
  const loadApplication = useCallback(
    async (uuid) => {
      if (shouldUseDatastore()) {
        ...
        const stayResult = await API.graphql(graphqlOperation(getStay, { id: uuid }));
        if (stayResult.data.getStay == null) {
          router.replace('/');
          return;
        }
        const applicationResult = await API.graphql(
          graphqlOperation(getApplication, { id: stayResult.data.getStay.applicationID })
        );
        ...
        const deserializedStay = deserializeModel(Stay, stayResult.data.getStay);
        setStay(deserializedStay);
        setApplication(application);
        setSourceApplication(application);
        if (application?.ServiceMember != null) {
  ...
  useEffect(() => {
    if (stay && stay.HotelBooked?.HotelBrand?.logo) {
      Storage.get(stay?.HotelBooked?.HotelBrand?.logo, {
        level: 'public',
      })
        .then((result) => {
          setHotelLogo(result);
