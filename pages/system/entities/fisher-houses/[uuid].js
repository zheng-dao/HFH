...
export default function FisherHouseEditPage() {
  ...
  const [archivedMedicalCenters, setArchivedMedicalCenters] = useState([]);
  const [name, setName] = useState('');
  const [nameValid, setNameValid] = useState(false);
  const [displayName, setDisplayName] = useState('');
  const [displayNameValid, setDisplayNameValid] = useState(false);
  const [selectedMedCenter, setSelectedMedCenter] = useState('');
  const [medCenterValid, setMedCenterValid] = useState(false);
  ...
  useEffect(() => {
    ...
    if (shouldUseDatastore()) {
      DataStore.query(
        Affiliation,
        (c) => c.status('ne', AFFILIATIONSTATUS.DRAFT).type('eq', AFFILIATIONTYPE.MEDICALCENTER),
        { page: 0, limit: 999999 }
      ).then((items) => {
        setMedicalCenters(items);
      });
    } else {
      API.graphql(
        ...
        .then((results) => {
          if (results.data.searchAffiliations.items.length > 0) {
            ...
            const activeMedicalCenters = sortedData.filter(
              (item) => item.status === AFFILIATIONSTATUS.ACTIVE
            );
            const arcMedicalCenters = sortedData.filter(
              (item) => item.status === AFFILIATIONSTATUS.ARCHIVED
            );
            setMedicalCenters(activeMedicalCenters);
            setArchivedMedicalCenters(arcMedicalCenters);
          }
        })
        .finally(() => {
          setLoadingCounter((prev) => prev - 1);
  ...
  const entityCityOnBlur = (e) => {
    setCityValid(validateRequired(e.target.value, 'The Fisher House City'));
  };
  ...
  const updateEntityState = (e) => {
    ...
    setStateValid(validateRequired(e.target.value, 'The Fisher House State'));
  };
  ...
  const updateEntityAssociatedAffiliation = (e) => {
    setEntity(
      Affiliation.copyOf(entity, (updated) => {
        updated.AssociatedAffiliation = medicalCenters.find((item) => item.id == e?.value);
      })
    );
    setMedCenterValid(validateRequired(e?.value));
  };
  ...
  const isValidForm = () => {
    ...
    const localMedCenterValid = validateRequired(entity.AssociatedAffiliation);
    setMedCenterValid(localMedCenterValid);
    return (
      localNameValid.valid &&
      localAddress1Valid.valid &&
      localZipValid.valid &&
      localMedCenterValid.valid
    );
  };
    ...
    if (!isValidForm()) {
      ...
      if (shouldUseDatastore()) {
        DataStore.save(newAffiliation).then((item) => {
          setEntity(item);
          setOriginalEntity(item);
          setMessage('Fisher House saved.');
          setTimeout(() => {
            router.push('/system/entities/fisher-houses');
          }, 1500);
        });
      } else {
        const {
        ...
        API.graphql(
          ...
          .catch(() => {
            setMessage('There was an error saving the Fisher House information.');
            setButtonDisabled(false);
            setIsWaiting(false);
          });
      }
  ...
  const deleteEntity = (e) => {
    e.preventDefault();
    if (
      confirm('Are you sure you want to delete this Fisher House? This action cannot be un-done.')
    ) {
      setButtonDisabled(true);
      setIsWaiting(true);
      if (shouldUseDatastore()) {
        ...
        API.graphql(
          ...
          .catch(() => {
            setMessage('There was an error deleting the Fisher House.');
            setButtonDisabled(false);
            setIsWaiting(false);
          });
      }
    ...
    if (!isValidForm()) {
      ...
      const newAffiliation = Affiliation.copyOf(entity, (updated) => {
        (updated.status = AFFILIATIONSTATUS.ARCHIVED),
          ...
          (updated.display_name = entity.display_name),
          (updated.address = entity.address),
          (updated.address_2 = entity.address2),
          (updated.city = entity.city),
          (updated.state = entity.state),
          (updated.zip = entity.zip),
          (updated.AssociatedAffiliation = entity.AssociatedAffiliation);
      ...
      if (shouldUseDatastore()) {
        DataStore.save(newAffiliation).then((item) => {
          setEntity(item);
          setMessage('Fisher House archived.');
          setTimeout(() => {
            router.push('/system/entities/fisher-houses');
          }, 1500);
        });
        ...
        API.graphql(
          ...
          .catch(() => {
            setMessage('There was an error archiving this Fisher House.');
            setButtonDisabled(false);
            setIsWaiting(false);
          });
      }
      ...
      if (shouldUseDatastore()) {
        DataStore.save(newAffiliation).then((item) => {
          ...
          if (entity?.status == AFFILIATIONSTATUS.ARCHIVED) {
            setMessage('Fisher House unarchived.');
          } else {
            setMessage('Fisher House approved.');
          }
          setTimeout(() => {
            router.push('/system/entities/fisher-houses');
          }, 1500);
        ...
        API.graphql(
          ...
          .catch(() => {
            setMessage('There was an error approving this Fisher House.');
            setButtonDisabled(false);
            setIsWaiting(false);
          });
      }
    ...
    <div className="page-container">
      ...
      <section className="main-content">
        <div className="container">
          <PageTitle prefix="System / " title="Fisher Houses" />
            ...
            <div className="main-column">
              <section className={appPaneClassNames}>
                <form id="system-pane">
                  ...
                  <div className="add-new-fisherhouse">
                    <Textfield
                      label="Fisher House Name"
                      wrapperClass="fisherhouse-name"
                    ...
                    <Selectfield
                      ...
                      wrapperClass="fisherhouse-medcenter"
                      inputValue={entity?.AssociatedAffiliation?.id}
                      inputRequired
                      options={
                        medicalCenters.find((item) => item.id === entity?.AssociatedAffiliation?.id) ?
                          medicalCenters.map((item) => {
                            return { value: item.id, label: item.name };
                          })
                          :
                          archivedMedicalCenters.find((item) => item.id === entity?.AssociatedAffiliation?.id) ?
                            [...medicalCenters, archivedMedicalCenters.find((item) => item.id === entity?.AssociatedAffiliation?.id)].map((item) => {
                              return { value: item.id, label: item.name };
                            })
                            :
                            medicalCenters.map((item) => {
                      ...
                      blankValue=""
                      isValid={medCenterValid.valid}
                      errorMessage={medCenterValid.message}
                      useReactSelect
                      useRegularSelect={false}
                      placeholder="Select a Medical Center..."
                      // inputDisabled={archivedMedicalCenters.findIndex((item) => item.id === entity?.AssociatedAffiliation?.id) !== -1}
                    />
                    ...
                    <div className="street-address-block">
                      <Textfield
                        ...
                        inputRequired
                        inputOnBlur={entityAddress1OnBlur}
                      ...
                      {zipValid.message && (
                        <Fragment>
                          <p className="errMsg">{[zipValid.message].filter((i) => i).join(' ')}</p>
                        </Fragment>
                      )}
                    </div>
                  </div>
