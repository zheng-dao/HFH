...
export default function FisherHousesSystemPage() {
    ...
    if (shouldUseDatastore()) {
      DataStore.save(
        new Affiliation({
          ...
          type: AFFILIATIONTYPE.FISHERHOUSE,
        })
      )
        .then((item) => {
          router.push('/system/entities/fisher-houses/' + item.id);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      API.graphql(
        graphqlOperation(createAffiliation, {
          input: { status: AFFILIATIONSTATUS.DRAFT, type: AFFILIATIONTYPE.FISHERHOUSE, name: '' },
        })
      )
        .then((result) => {
          setIsWaiting(false);
          router.push('/system/entities/fisher-houses/' + result.data.createAffiliation.id);
        })
        .catch((err) => {
          ...
          setIsWaiting(false);
          setMessage(
            'There was an error creating a new Fisher House. Please refresh the page and try again.'
          );
        });
    }
  };
  ...
  const loadData = useCallback(
    (token) => {
      ...
      filters.type = { eq: AFFILIATIONTYPE.FISHERHOUSE };
    ...
    if (shouldUseDatastore()) {
      DataStore.query(
        ...
        for (const result of results) {
          output.push({
            name: (
              <Link href={'/system/entities/fisher-houses/' + result.id}>
                <a>{result.name}</a>
              </Link>
            ),
            city: (
      ...
      loadData('').then((results) => {
        ...
        for (const result of results.data.searchAffiliations.items.filter(
          (item) => !item['_deleted']
        )) {
          output.push({
            name: (
