...
export default function MilitaryBasesSystemPage() {
    ...
    if (shouldUseDatastore()) {
      DataStore.save(
        new Affiliation({
          ...
          type: AFFILIATIONTYPE.BASE,
        })
      )
        .then((item) => {
          router.push('/system/entities/military-bases/' + item.id);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      API.graphql(
        graphqlOperation(createAffiliation, {
          input: { status: AFFILIATIONSTATUS.DRAFT, type: AFFILIATIONTYPE.BASE, name: '' },
        })
      )
        .then((result) => {
          setIsWaiting(false);
          router.push('/system/entities/military-bases/' + result.data.createAffiliation.id);
        })
        .catch((err) => {
          console.log('Caught error', err);
          setMessage(
            'There was an error creating a new Military Base. Please refresh the page and try again.'
          );
  ...
  const loadData = useCallback(
    (token) => {
      ...
      filters.type = { eq: AFFILIATIONTYPE.BASE };
    ...
    if (shouldUseDatastore()) {
      DataStore.query(
        ...
        for (const result of results) {
          output.push({
            name: (
              <Link href={'/system/entities/military-bases/' + result.id}>
                <a>{result.name}</a>
              </Link>
            ),
            city: (
