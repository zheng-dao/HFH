...
export default function OrganizationsSystemPage() {
  ...
  const addNewClick = (e) => {
    ...
    if (shouldUseDatastore()) {
      DataStore.save(
        ...
        .then((item) => {
          router.push('/system/entities/organizations/' + item.id);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      API.graphql(
        graphqlOperation(createAffiliation, {
          input: { status: AFFILIATIONSTATUS.DRAFT, type: AFFILIATIONTYPE.ORGANIZATION, name: '' },
        })
      )
        .then((result) => {
          setIsWaiting(false);
          router.push('/system/entities/organizations/' + result.data.createAffiliation.id);
        })
        .catch((err) => {
          console.log('Caught error', err);
          setMessage(
            'There was an error creating a new Organization. Please refresh the page and try again.'
          );
  ...
  const loadData = useCallback(
    (token) => {
      ...
      filters.type = { eq: AFFILIATIONTYPE.ORGANIZATION };
    ...
    if (shouldUseDatastore()) {
      DataStore.query(
        ...
        for (const result of results) {
          output.push({
            name: (
              <Link href={'/system/entities/organizations/' + result.id}>
                <a>{result.name}</a>
              </Link>
            ),
            city: (
