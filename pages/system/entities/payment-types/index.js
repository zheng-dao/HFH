...
import { PAYMENTTYPESTATUS, PAYMENTTYPETYPE } from '@src/API';
...
import { listPaymentTypes, searchPaymentTypes } from '@src/graphql/queries';
import { createPaymentType } from '@src/graphql/mutations';
import useDialog from '@contexts/DialogContext';
import useButtonWait from '@contexts/ButtonWaitContext';

export default function PaymentTypesSystemPage() {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterSelections, setFilterSelections] = useState(
    Object.values(PAYMENTTYPESTATUS)
      .map((val) => {
        return val;
      })
  ...
  const filterOptions = Object.values(PAYMENTTYPESTATUS)
    .map((val) => {
      return {
        key: val,
        value: val,
  ...
  const addNewClick = (e) => {
    ...
    if (shouldUseDatastore()) {
      DataStore.save(
        new PaymentType({
          status: PAYMENTTYPESTATUS.DRAFT,
        })
      )
        .then((item) => {
          router.push('/system/entities/payment-types/' + item.id);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      API.graphql(
        graphqlOperation(createPaymentType, { input: { status: PAYMENTTYPESTATUS.DRAFT } })
      )
        .then((results) => {
          setIsWaiting(false);
          router.push('/system/entities/payment-types/' + results.data.createPaymentType.id);
        })
        .catch((err) => {
          console.log('Caught error', err);
          setMessage(
            'There was an error creating a new Payment Type. Please refresh the page and try again.'
          );
  ...
  const tableColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        className: '',
        sortType: (a, b) => {
          return a.values.description?.props?.children?.props?.children?.localeCompare(
            b.values.description?.props?.children?.props?.children,
            undefined,
            { sensitivity: 'base' }
          );
        },
      },
      {
        ...
        sortType: (a, b) => {
          let sortOrder = [PAYMENTTYPESTATUS.ACTIVE, PAYMENTTYPESTATUS.ARCHIVED];
          return (
  ...
  const loadData = useCallback(
    (token) => {
      ...
      return API.graphql(graphqlOperation(searchPaymentTypes, variables));
    },
  ...
  useEffect(() => {
    if (shouldUseDatastore()) {
      DataStore.query(
        PaymentType,
        (m) => m.status('ne', PAYMENTTYPESTATUS.DRAFT).status('ne', null),
        {
          page: currentPage,
          limit: 999999,
        }
      ...
      loadData('').then((results) => {
        let output = [];
        setNextToken(results.data.searchPaymentTypes.nextToken);
        setTotalCount(results.data.searchPaymentTypes.total || 0);
        results.data.searchPaymentTypes.items.sort((a, b) =>
          a.name?.localeCompare(b.name, undefined, { sensitivity: 'base' })
        );
        for (const result of results.data.searchPaymentTypes.items.filter(
          (item) => !item['_deleted']
        )) {
          output.push({
            name: (
  ...
  const _tableDataMemo = useMemo(() => {
    ...
    //   return (
    //     searchTerm == '' ||
    //     (item.raw_name && item.raw_name.toLowerCase().includes(searchTerm.toLowerCase()))
    //   );
    // });
  }, [tableData]);
