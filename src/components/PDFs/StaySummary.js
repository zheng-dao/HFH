...
const StaySummary = ({ isAdministrator, stay, maskStayStatus }) => {
  ...
  let summaryBoxThree = [
    {
      ...
      value: mapEnumValue(stay?.room_type_actual),
    },
  ];
  if (stay?.room_type_actual == ROOMTYPE.OTHER) {
  ...
  return (
    <View>
      ...
      <ApplicationSummary sectionTitle="" listItems={summaryBoxOne} hideTitle={true} />
      ...
      {
        isAdministrator && <ApplicationSummary sectionTitle="Reconciliation" listItems={reconciliationSummary} />
      }
    </View>
  );
};
