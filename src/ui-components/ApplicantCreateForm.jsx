...
export default function ApplicantCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setBase_assigned_to(initialValues.base_assigned_to);
    setRelation_to_service_member(initialValues.relation_to_service_member);
    setReferrer_date(initialValues.referrer_date);
    setUser_type(initialValues.user_type);
    setMiddle_initial(initialValues.middle_initial);
    setPatient_type(initialValues.patient_type);
    setExtension(initialValues.extension);
    setFamily_lodge(initialValues.family_lodge);
    setLocation_name(initialValues.location_name);
    setLocation_address(initialValues.location_address);
    setLodging_explanation(initialValues.lodging_explanation);
    setAffiliation_type(initialValues.affiliation_type);
    setCollected_outside_fisherhouse(initialValues.collected_outside_fisherhouse);
    setErrors({});
  };
  const validations = {
    first_name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new Applicant(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, 'ApplicantCreateForm')}
    >
      <TextField
        label="First name"
        isRequired={false}
