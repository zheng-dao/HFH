...
export default function GuestUpdateForm(props) {
  const {
    id,
    guest,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...guestRecord };
    ...
    setUnder_age_three(cleanValues.under_age_three);
    setErrors({});
  };
  const [guestRecord, setGuestRecord] = React.useState(guest);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Guest, id) : guest;
      setGuestRecord(record);
    };
    queryData();
  }, [id, guest]);
  React.useEffect(resetStateValues, [guestRecord]);
  const validations = {
    first_name: [],
    relationship: [],
    middle_initial: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Guest.copyOf(guestRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, 'GuestUpdateForm')}
    >
      <TextField
        label="First name"
        isRequired={false}
