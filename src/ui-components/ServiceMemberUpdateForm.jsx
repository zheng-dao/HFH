...
export default function ServiceMemberUpdateForm(props) {
  const {
    id,
    serviceMember,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...serviceMemberRecord };
    ...
    setOn_military_travel_orders(cleanValues.on_military_travel_orders);
    setOther_patient(cleanValues.other_patient);
    setLodging_explanation(cleanValues.lodging_explanation);
    setUnidentified_explanation(cleanValues.unidentified_explanation);
    setErrors({});
  };
  const [serviceMemberRecord, setServiceMemberRecord] = React.useState(serviceMember);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(ServiceMember, id) : serviceMember;
      setServiceMemberRecord(record);
    };
    queryData();
  }, [id, serviceMember]);
  React.useEffect(resetStateValues, [serviceMemberRecord]);
  const validations = {
    first_name: [],
    middle_initial: [],
    last_name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            ServiceMember.copyOf(serviceMemberRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, 'ServiceMemberUpdateForm')}
    >
      <TextField
        label="First name"
        isRequired={false}
      ...
      <TextField
        ...
        isRequired={false}
        isReadOnly={false}
        defaultValue={unidentified_explanation}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
