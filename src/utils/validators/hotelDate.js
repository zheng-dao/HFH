export default function validateHotelDate(value) {
  ...
  if (value && value.length > 0 && !isAWSDateRegex.test(value)) {
    return {
      valid: false,
      message:
  ...
  if (dateToCheck.getFullYear() > (todayDate.getFullYear() + 1)) {
    return {
      valid: false,
      message:
        'Please enter a date from 1990 through next year (' + (todayDate.getFullYear() + 1) + ').',
  ...
  if (dateToCheck.getFullYear() < 1990) {
    // Year is less than 1990. Throw error.
    return {
      valid: false,
      message:
        'Please enter a date from 1990 through next year (' + (todayDate.getFullYear() + 1) + ').',
    };
  } else if (dateToCheck.getFullYear() - todayDate.getFullYear() > 1) {
    return {
      valid: false,
      message:
        'Please enter a date from 1990 through next year (' + (todayDate.getFullYear() + 1) + ').',
